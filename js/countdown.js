...
var g_cd = {
    ...
    registerActions: () => {
        ...
        registerAction('cd_setDailyLess', (dom, action, params) => {
            prompt(_l('一天最少计时'), 8 * 60, { numberOnly: 1 }).then((d) => {
                var min = parseInt(d.text);
                if (min > 0) {
                    g_config.countdown_dailyLess = min * 60;
                    local_saveJson('config', g_config);
                    g_cd.initTodaySpend(g_cd.todaySpend);
                }
            });
        });
    ...
    initHtml: (day) => {
        ...
        g_cd.initTodaySpend(all);
    },
    initTodaySpend: (all) => {
        g_cd.todaySpend = all;
         g_cd.bar.html(all > 0 ? `
            <h5 data-action="cd_setDailyLess">` + _l('今日总共', getTime(all)) + `(` + (parseInt(all / (g_config.countdown_dailyLess || 86400) * 100)) + `%)</h5>
        ` : '');
    },
